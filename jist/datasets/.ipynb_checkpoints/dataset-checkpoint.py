...
import torchvision.transforms as transforms#pca
from itertools import product#pca


def path_to_pil_img(path):
    return Image.open(path).convert("RGB")

class PCADataset(data.Dataset):#from cricavpr
    def __init__(self, args, datasets_folder="/mnt/datasets", dataset_folder="pitts30k/images/train"):
    ...
    def __getitem__(self, index):
        img = self.base_transform(path_to_pil_img(self.images_paths[index]))
...
class PCADataset_sequence(data.Dataset):#from vg_transformer
...
class BaseDataset(data.Dataset):
    ...
    def init_data(self, cities, split, pos_thresh, neg_thresh):
        ...
        if ('msls' in self.dataset_folder) and (split == 'train'):#我改的哈，原本没有msls的限制
            save_dict = {
...
def build_sequences(folder, seq_len=3, cities='', desc='loading'):
    ...
    for seq in tqdm(seqs_folders, ncols=100, desc=desc):
        ...
        for idx, frame_num in enumerate(frame_nums):
            ...
            if (np.diff(frame_nums[sorted_idx_frames][seq_idx]) == 1).all():#这里解决空挡问题，前两个和后两个都得保证有，当前帧在中心位置
...
class TrainDataset(BaseDataset):
    def __init__(self, cities='', dataset_folder="datasets", split="train", base_transform=None,
                 seq_len=3, pos_thresh=25, neg_thresh=25, infer_batch_size=8,n_gpus=1,#only for seqvlad
                ...
                for images, indexes, _ in tqdm(subset_dl, desc="cache feat extraction", ncols=100):
                    ...
                    if (images.shape[0] % (self.seq_len * self.n_gpus) != 0) and self.n_gpus > 1:
                        ...
                        for sequence in range(images.shape[0] // self.seq_len):
                            ...
                            seq_images = images[n_seq: n_seq + self.seq_len].to('cuda:0')
                            _,frames_features = model.module(seq_images)
                            aggregated_features = model.module.aggregate(frames_features)
                            if isinstance(aggregated_features, list):#补
                                aggregated_features = aggregated_features[0]#补

                            cache[indexes[sequence], :] = aggregated_features.cpu().numpy()

                        model = model.cuda()
                    elif self.n_gpus > 1:
                        _,frames_features = model(images.to(self.device))
                        aggregated_features = model.module.aggregate(frames_features)
    ...
    def compute_triplets_partial(self, model):
        ...
        if(self.n_gpus > 1):
            cache = self.compute_cache(model, subset_ds, cache_shape=[len(self), model.module.aggregation_dim])
        else:
