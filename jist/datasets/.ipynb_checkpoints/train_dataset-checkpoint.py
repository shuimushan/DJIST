...
class CosplaceTrainDataset(torch.utils.data.Dataset):
    def __init__(self, args, dataset_folder, M=10, alpha=30, N=5, L=2,
        ...
        # dataset_name should be "processed" (if you're using SF-XL)
        ...
        self.base_path = dataset_folder[:dataset_folder.find(dataset_name)]
        ...
        if self.augmentation_device == "cpu":
            self.transform = T.Compose([
                    T.ColorJitter(brightness=args.brightness,
                                  ...
                                  hue=args.hue),
                   # T.RandomResizedCrop([512, 512], scale=[1-args.random_resized_crop, 1]),原本是512，512，我的训练需要，改成224，224
    ...
    def initialize(dataset_folder, M, N, alpha, L, min_images_per_class, filename):
        ...
        all_paths = os.path.join(dataset_folder, '../../all_images_paths.txt')
        ...
        class_id__group_id = [CosplaceTrainDataset.get__class_id__group_id(*m, M, alpha, N, L)
