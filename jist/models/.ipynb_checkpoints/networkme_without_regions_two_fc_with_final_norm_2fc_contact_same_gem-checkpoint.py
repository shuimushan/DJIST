...
class Me(nn.Module):
    ...
    def forward(self, x):
        ...
        x_gem_fc = self.fc(x_gem)
        x_gem_fc = F.normalize(x_gem_fc, p=2, dim=1)
        #x = x.view(B, C, int(np.sqrt(HW)), int(np.sqrt(HW)))


        
        return x_gem_fc,[x_gem_fc,x_gem]
...
class JistModelMe_without_regions_two_fc_with_final_norm_2fc_contact_same_gem(nn.Module):
    ...
    def aggregate(self, frames_features):
        ...
        if self.agg_type == "seqgem":
            ...
            #aggregated_features2 = self.gem2(frames_features[1])
            #aggregated_features2 = F.normalize(aggregated_features2, p=2, dim=1)
            aggregated_features2 = einops.rearrange(frames_features[1], "(b sl) d -> b sl d", sl=self.seq_length)
