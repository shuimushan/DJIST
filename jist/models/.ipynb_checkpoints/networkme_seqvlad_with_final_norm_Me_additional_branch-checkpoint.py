...
class JistModelMe_seqvlad_with_final_norm_Me_additional_branch(nn.Module):
    def __init__(self, args, agg_type="seqvlad"):
        super().__init__()
        self.model = Me()
        self.seqvlad1 = SeqVLAD(seq_length=args.seq_length, dim=768,
                                     transf_backbone=False)
        self.seqvlad2 = SeqVLAD(seq_length=args.seq_length, dim=768,
                                     transf_backbone=False)        
        ...
        self.aggregation_dim = self.fc_output_dim * 64*2
    ...
    def aggregate(self, frames_features):
        aggregated_features1 = self.seqvlad1(frames_features[0])
        aggregated_features2 = self.seqvlad2(frames_features[1])
