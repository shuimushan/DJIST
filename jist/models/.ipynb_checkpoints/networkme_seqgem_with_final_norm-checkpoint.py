...
class JistModelMe_seqgem_with_final_norm(nn.Module):
    def __init__(self, args, agg_type="concat"):
        ...
        if agg_type == "temporal_mix":#缺点是只能针对固定seq_len
            self.aggregation_dim = self.fc_output_dim*8
            self.temporal_mix = nn.Sequential(*[Mixer(fc_in_channels=self.fc_output_dim, in_dim=self.seq_length, mlp_ratio=1,in_channels=self.fc_output_dim) for _ in range(2)])
            self.temporal_proj = nn.Linear(self.seq_length, 8)
            self.channel_proj = nn.Linear(self.fc_output_dim, self.fc_output_dim)
        if agg_type == "temporal_mix_lower_dim":
            self.aggregation_dim = self.fc_output_dim
            self.temporal_mix = nn.Sequential(*[Mixer(fc_in_channels=self.fc_output_dim, in_dim=self.seq_length, mlp_ratio=1,in_channels=self.fc_output_dim) for _ in range(2)])
    ...
    def forward(self, x):
        x= self.model(x)
        #x = self.aggregate(x)#仅为了验证参数量和FLOPs
        return x,x
    ...
    def aggregate(self, frames_features):
        ...
        if self.agg_type in ["temporal_mix","temporal_mix_lower_dim"]:
