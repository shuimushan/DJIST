...
class Me(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.normalize(x, p=2, dim=1)
        x_gem = self.gem1_without_norm(x)
        x_gem = x_gem.unsqueeze(dim=1)
        x = einops.rearrange(x, "b c h w -> b (h w) c")# b 256 c
        atten1 = torch.matmul(x_gem, x.transpose(1,2)).squeeze()#256
        ...
        return x_gem,[x_gem,x_addition,atten1]
...
class JistModelMe_with_final_norm_with_additional_branch_with_attention3_new(nn.Module):
    ...
    def forward(self, x):
        #x1,x2 = x#仅为计算参数和FLOPs
        #x2=self.aggregate(x2)
        return self.model(x)
    ...
    def aggregate(self, frames_features):
        ...
        if self.agg_type == "seqgem":
            ...
            aggregated_features1 = einops.rearrange(frames_features[0], "(b sl) d -> b sl d", sl=self.seq_length)
            aggregated_features1 = self.seq_gem1(aggregated_features1)
            aggregated_features1 = F.normalize(aggregated_features1, p=2, dim=1)
            atten1 = frames_features[2]
