...
class Mixer(nn.Module):
    def __init__(self, fc_in_channels=768, in_dim=16*16, mlp_ratio=1,in_channels=2048):
        super().__init__()
        self.norm = nn.LayerNorm(fc_in_channels)
        self.mix = nn.Sequential(
            nn.Linear(in_dim, int(in_dim * mlp_ratio)),
    ...
    def forward(self, x):#输入x为[B,C,seq_len]
        x = x + self.mix(self.norm(x.permute(0,2,1)).permute(0,2,1))
        x = x.permute(0,2,1)
        x = x + self.mlp(x)
        x = x.permute(0,2,1)#输出x为[B,C,seq_len]
        return x
...
class JistModelMe_seqgem_with_final_norm_not_fc_continue_different_gem_gem512_sequential512(nn.Module):
    def __init__(self, args, agg_type="concat"):
        ...
        if agg_type == "concat":
            self.aggregation_dim = self.features_dim * args.seq_length
        if agg_type == "mean":
            self.aggregation_dim = self.fc_output_dim
        if agg_type == "max":
            self.aggregation_dim = self.fc_output_dim
        if agg_type == "conv1d":
            self.conv1d = torch.nn.Conv1d(self.features_dim, self.fc_output_dim, self.seq_length)
            self.aggregation_dim = self.fc_output_dim
        if agg_type in ["simplefc", "meanfc"]:
            self.aggregation_dim = self.fc_output_dim
            self.final_fc = torch.nn.Linear(self.features_dim * args.seq_length, self.fc_output_dim, bias=False)
        if agg_type == "meanfc":
            ...
            for i in range(self.fc_output_dim):
                for j in range(args.seq_length):
                    weights[i, j * self.features_dim + i] = 1 / args.seq_length
            self.final_fc.weight = torch.nn.Parameter(weights)
        if agg_type == "seqgem":
            self.aggregation_dim = self.fc_output_dim
            self.seq_gem = SeqGeM()
            self.fc = nn.Linear(self.features_dim, self.fc_output_dim)
        if agg_type == "temporal_mix":#缺点是只能针对固定seq_len
            self.aggregation_dim = self.fc_output_dim
            self.temporal_mix = nn.Sequential(*[Mixer(fc_in_channels=self.features_dim, in_dim=self.seq_length, mlp_ratio=1,in_channels=self.features_dim) for _ in range(2)])
            self.temporal_proj = nn.Linear(self.seq_length, 1)
            self.channel_proj = nn.Linear(self.features_dim, self.fc_output_dim)
    ...
    def aggregate(self, frames_features):
        if self.agg_type == "concat":
            ...
            return  F.normalize(concat_features, p=2, dim=-1)
        if self.agg_type == "mean":
            aggregated_features = einops.rearrange(frames_features, "(b sl) d -> b sl d", sl=self.seq_length)
            return F.normalize(aggregated_features.mean(1), p=2, dim=-1)
        if self.agg_type == "max":
            aggregated_features = einops.rearrange(frames_features, "(b sl) d -> b sl d", sl=self.seq_length)
            return F.normalize(aggregated_features.max(1)[0], p=2, dim=-1)
        ...
        if self.agg_type in ["simplefc", "meanfc"]:
            concat_features = einops.rearrange(frames_features, "(b sl) d -> b (sl d)", sl=self.seq_length)
            return F.normalize(self.final_fc(concat_features), p=2, dim=-1)
        if self.agg_type == "seqgem":
            #aggregated_features = F.normalize(frames_features, p=2, dim=-1)已经在GeMFc里normalize过了，这里不再重复
            aggregated_features = einops.rearrange(frames_features, "(b sl) d -> b sl d", sl=self.seq_length)
            aggregated_features = self.seq_gem(aggregated_features)
            aggregated_features = self.fc(aggregated_features)
            return F.normalize(aggregated_features, p=2, dim=-1)
        if self.agg_type == "temporal_mix":
            aggregated_features = einops.rearrange(frames_features, "(b sl) d -> b d sl", sl=self.seq_length)
            #print(aggregated_features.shape)
            aggregated_features = self.temporal_mix(aggregated_features)#[B,C,seq_len]
            aggregated_features = self.channel_proj(aggregated_features.permute(0,2,1))
            aggregated_features = self.temporal_proj(aggregated_features.permute(0,2,1))
            return F.normalize(aggregated_features.flatten(1), p=2, dim=-1)
