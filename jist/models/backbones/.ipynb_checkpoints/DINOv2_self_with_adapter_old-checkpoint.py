...
class DinoV2_self(nn.Module):
    ...
    def __init__(self, model_name: _DINO_V2_MODELS, layer1: int = 39,  facet1: _DINO_FACETS = "value", use_cls=False,
                 norm_descs=True, device: str = "cuda", pretrained=True, out_indices=[8, 9, 10, 11]) -> None:
        """
            Parameters:
        ...
        self.layer1 = Adapter(768)#.to(self.device)
        self.layer2 = Adapter(768)
        self.layer3 = Adapter(768)
        self.layer4 = Adapter(768)
        #self.adapt_layers.append(self.layer1)
        #self.adapt_layers.append(self.layer2)
        #self.adapt_layers.append(self.layer3)
        #self.adapt_layers.append(self.layer4)
    ...
    def forward(self, x, masks=None):
        ...
        x=self.dino_model.get_intermediate_layers(x,n=self.out_indices,reshape = False)
        #print(x[0].shape)
        #exit()
        x_list = []
        for i in range(len(x)):
            if(i==0):  xi = self.layer1(x[i])
            if(i==1):  xi = self.layer2(x[i])
            if(i==2):  xi = self.layer3(x[i])
            if(i==3):  xi = self.layer4(x[i])
            bs, f, c = xi.shape
            xi = xi.view(bs, int(np.sqrt(f)), int(np.sqrt(f)), c).permute(0, 3, 1, 2)
            x_list.append(xi)
        x = x_list
        x = torch.cat(x, dim=1)
        #print(x.shape)
