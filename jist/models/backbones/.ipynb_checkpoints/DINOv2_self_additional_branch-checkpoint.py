...
class DinoV2_self_additional_branch(nn.Module):
    """
    ...
    def forward(self, x, masks=None):
        # 获取所有指定中间层的输出（序列格式）
        # 注意：这里设置 return_class_token=True 会返回元组列表
        ...
        def to_feature_map(patches):
         ...
         # === 主分支：原始模型的四层输出 ===
        main_feature_maps = [to_feature_map(seq) for seq in feature_sequences]
        main_output = torch.cat(main_feature_maps, dim=1)  # 在通道维度拼接
        
        # === 附加分支：组合原始层和分支层 ===
        # 前两层使用原始模型的输出
        branch_feature_maps = [
            to_feature_map(feature_sequences[0]),  # 第 out_indices[0] 层
            to_feature_map(feature_sequences[1])   # 第 out_indices[1] 层
        ]
        
        # 后两层使用分支block生成
        # 从原始模型的第 out_indices[1] 层开始
        branch_seq = intermediate_outputs[1][0]  # 获取特征序列 [B, num_patches, C]
        
       # 通过两个分支block
        branch_layer1 = self.branch_block1(branch_seq)
        branch_layer2 = self.branch_block2(branch_layer1)
        
        # 转换为特征图
        branch_feature_maps.append(to_feature_map(branch_layer1))
        branch_feature_maps.append(to_feature_map(branch_layer2))
        
        branch_output = torch.cat(branch_feature_maps, dim=1)
