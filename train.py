...
checkpoint = torch.load("./weights/SuperCricaVPR-B.pth")
#print(model.model)
...
params_conv = model.model.conv.state_dict()
model.model.conv_addition.load_state_dict(params_conv)
params_tokenmix = model.model.tokenmix.state_dict()
model.model.tokenmix_addition.load_state_dict(params_tokenmix)
...
gpu_augmentation = T.Compose([
            ...
            utils.augmentations.DeviceAgnosticRandomResizedCrop([224, 224],
                                                                scale=[1-args.random_resized_crop, 1]),#被我改成了224，224 原本是512，512
...
for epoch_num in range(start_epoch_num, args.epochs_num):
    ...
    sequence_mean_loss_me = torchmetrics.MeanMetric()
    ...
    for num_sub_epoch in range(args.num_sub_epochs):
        ...
        for images, _, _ in tqdm_bar:
            ...
            if args.lambda_triplet != 0:
                ...
                with torch.cuda.amp.autocast():
                    _, features = model(images.to(args.device))
                    features_list = model.aggregate(features)
                    features = features_list[0]
                    atten1 = features_list[1]
                    atten2 = features_list[2]
                    ...
                    for d in range(args.train_batch_size * args.nNeg):
                        sequence_loss_me+=(1+torch.sum(atten1[d]*atten2[d]))
                del images, features, features_list, atten1, atten2
                sequence_loss /= (args.train_batch_size * args.nNeg)
                sequence_loss *= args.lambda_triplet
                sequence_loss_me /= (args.train_batch_size * args.nNeg)
                sequence_loss_me *= args.lambda_triplet_me
                scaler.scale(sequence_loss+sequence_loss_me).backward()
                sequence_mean_loss.update(sequence_loss.item())
                sequence_mean_loss_me.update(sequence_loss_me.item())
                del sequence_loss
                del sequence_loss_me
            else:
                sequence_mean_loss.update(-1)
                sequence_mean_loss_me.update(-1)
            ...
            scaler.update()
            tqdm_bar.set_description(f"seq_loss: {sequence_mean_loss.compute():.4f} - seq_loss_me: {sequence_mean_loss_me.compute():.4f} - cos_loss: {cosplace_mean_loss.compute():.2f}")
        ...
        logging.debug(f"Epoch[{epoch_num:02d}]({num_sub_epoch + 1}/{args.num_sub_epochs}): " +
                      ...
                      f"epoch sequence_me loss = {sequence_mean_loss_me.compute():.4f} - " +
